...
class TransformerEdgePolicy(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.encoder_layer = RelativePosTransformerLayer(args['transformer_model_dim'], args['nhead'], dim_feedforward=args['transformer_fc_dim'], hard_attention = self.hard_attention, train_attention = self.train_attention, only_obs_edge=self.only_obs_edge, device = self.device)

        self.na_params.extend(self.encoder_layer.na_params)


        if self.n_hops >= 2:
            self.encoder_layer_2 = RelativePosTransformerLayer(args['transformer_model_dim'], args['nhead'], dim_feedforward=args['transformer_fc_dim'], hard_attention = self.hard_attention,  device = self.device)
            self.na_params.extend(self.encoder_layer_2.na_params)

        if self.n_hops >= 3:
            self.encoder_layer_3 = RelativePosTransformerLayer(args['transformer_model_dim'], args['nhead'], dim_feedforward=args['transformer_fc_dim'], hard_attention = self.hard_attention,  device = self.device)
    ...
    def forward(self, input, edge_input, prog_input = None, need_weights=False):
        # Input (S, N, d_in)
        # edge_input (S, N^2, d_in_edge)
        input = input.to(self.device)
        x_kq = F.relu(self.fc1(input)) # (S, N, d_model)
        #x_kq = torch.unsqueeze(x_kq, 1) # (N, 1, d_model)
        if self.sep_key_val:
            x_v = F.relu(self.fc1_v(input)) # (S, N, d_model)
            #x_v = torch.unsqueeze(x_v, 1) # (N, 1, d_model)
        else:
            x_v = x_kq # (N, 1, d_model)

        if self.use_internal_state and self.internal_state != None:
            x_v = x_v + self.internal_state


        edge_input = edge_input.to(self.device)
        edge_k = F.relu(self.fc_edge_k(edge_input)) # (S, N^2, d_model)
        edge_v = F.relu(self.fc_edge_v(edge_input)) # (S, N^2, d_model)

        if self.sep_self_edge:
            assert(False) # not compatible with S
            N = input.size()[1]
        ...
        x, att_weights1 = self.encoder_layer(x_kq, x_v, edge_k, edge_v,  dt_att_weights[0], soft_with_prog=self.use_soft_with_prog) # (S, N, d_model)
        if self.n_hops >= 2:
            assert(not self.sep_key_val) # not yet supported with different k and v encoding, and S
            x, att_weights2 = self.encoder_layer_2(x_kq, x, edge_k, edge_v, dt_att_weights[1], soft_with_prog=self.use_soft_with_prog) # (S, N, d_model)

        if self.n_hops >= 3:
            assert(not self.sep_key_val) # not yet supported with different k and v encoding
            x, att_weights3 = self.encoder_layer_3(x_kq, x, edge_k, edge_v, dt_att_weights[2], soft_with_prog=self.use_soft_with_prog) # (N, 1, d_model)
...
class RelativePosTransformerLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", hard_attention=False, train_attention = True, only_obs_edge=False, device = None):
        ...
        self.self_attn = EdgeMultiheadAttention(d_model, nhead, dropout=dropout, hard_attention = hard_attention,  only_obs_edge=only_obs_edge)
    ...
    def forward(self, src_kq, src_v, edge_k, edge_v, dt_att_weights, need_weights=True, soft_with_prog=False):
        """
        src: (S, N,  d_model)
        edge_k, edge_v: (S, N^2, d_model)
        ...
        if dt_att_weights == None:
            ...
            bsz, N, _ = src_kq.size()
            mask = torch.zeros((bsz, N, N)).to(self.device) > 1 # False
            # fill diagonal with True
            Y, X = np.meshgrid(np.arange(N), np.arange(bsz))
            X = X.reshape(N*bsz)
            Y = Y.reshape(N*bsz)

            mask[X, Y, Y] = True # mask self edges
            ...
            mask = torch.zeros((bsz, N, N)).to(self.device) > 1
...
class EdgeMultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, hard_attention = False, only_obs_edge=False, dropout=0., bias=True):
    ...
    def forward(self, query, key, edge_k, key_padding_mask=None):
        ...
        q = q.contiguous().view(bsz * num_heads, seq_len,  head_dim)#.transpose(0, 1)
        k = k.contiguous().view(bsz * num_heads, -1,  head_dim)#.transpose(0, 1)
        # q, k: (N*h, S, E/h)
        ...
        # edge k: (S^2, N, E)
        edge_k = edge_k.contiguous().view(bsz * num_heads, seq_len*seq_len,  head_dim)#.transpose(0, 1)
        ...
        if self.hard_attention:
            ...
            #max_weight = (torch.max(attn_output_weights))
            #attn_output_weights[attn_output_weights < 0.7*max_weight] = 0
            #attn_output_weights[attn_output_weights > 1e-2] = 1
            num_agents_to_ignore = math.floor(seq_len - 2)
            ...
            degree_so_far = torch.zeros(seq_len)

            attn_weights = attn_output_weights[0]
            attn_weights1 = F.softmax(attn_weights, dim=-1)

            bidir_edge_weights = (attn_weights1 + torch.t(attn_weights1)).reshape((seq_len * seq_len))
            sort_idx = bidir_edge_weights.argsort().flip(0)


            tensor = torch.ones((2,), dtype=torch.float32)
            new_attn_weights = tensor.new_full((1, seq_len, seq_len), float('-inf'),  dtype=torch.float32)
            for idx in sort_idx:
                x = idx//seq_len
                y = idx%seq_len
                if  degree_so_far[x] < max_degree and degree_so_far[y] < max_degree:
                    new_attn_weights[0][x][y] = attn_weights[x][y]
                    new_attn_weights[0][y][x] = attn_weights[y][x]
            ...
            attn_output_weights = new_attn_weights'''
        ...
        if self.hard_attention:
            ...
            attn_output_weights[attn_output_weights < 1e-2] = 0

            sums = attn_output_weights.sum(axis = 2) + 1e-4
            sums = sums.reshape((1, seq_len, 1))
            attn_output_weights = attn_output_weights/sums
...
class EdgeMultiheadValue(nn.Module):
    ...
    def forward(self, attn_output_weights, value, edge_v):
        ...
        bsz, seq_len, embed_dim = value.size()
        ...
        v = v.contiguous().view( bsz * num_heads, -1, head_dim)#.transpose(0, 1)
        #v: (N*h, S, E/h)
        ...
        attn_output = attn_output.contiguous().view(bsz, seq_len,  embed_dim) # (N, S, E)


        attn_output = F.linear(attn_output, self.out_proj.weight, self.out_proj.bias) # (N, S,, E)
